...
function workflowSelector() {
  ...
  const shared = sharedEnvs(project)
  return `<section class="workflow-env"><div class="workflow-env-head"><b>Workflow</b><small>✓ Zapisane</small></div><label>Venv<select id="workflowEnvironment" ${canEdit()?'':'disabled'}><option value="" ${workflow.environmentId?'':'selected'}>Dziedzicz z projektu · ${esc(inherited?.name || '')}</option><optgroup label="Projektowe">${shared.map(env=>`<option value="${env.id}" ${workflow.environmentId===env.id?'selected':''}>${esc(env.name)} · Python ${esc(env.python)}</option>`).join('')}</optgroup>${own.length?`<optgroup label="Tylko ten workflow">${own.map(env=>`<option value="${env.id}" ${workflow.environmentId===env.id?'selected':''}>${esc(env.name)} · Python ${esc(env.python)}</option>`).join('')}</optgroup>`:''}</select></label><div class="workflow-env-active"><i class="env-dot ${active?.status || 'broken'}"></i><small>${esc(active?.name || 'brak')} · Python ${esc(active?.python || '?')} · ${esc(active?.path || '')}</small></div><button data-env-action="add-workflow" ${canEdit()?'':'disabled'}>+ Własny venv</button><small class="hint">Ten wybór jest zapisany w workflow i ma być używany przy ręcznym oraz zaplanowanym uruchomieniu.</small></section>`
}
...
function inject() {
  ...
  if (state.route === 'designer') {
    ...
    if (inspector && !inspector.querySelector('.workflow-env')) {
      const title = inspector.querySelector('.panel-title')
      if (title) title.insertAdjacentHTML('afterend', workflowSelector())
      else inspector.insertAdjacentHTML('afterbegin', workflowSelector())
    }
  }
  bindEnv()
}
...
function bindEnv() {
  ...
  if (p && !p.dataset.bound) {
    p.dataset.bound='1'
    p.addEventListener('change',()=>{project.environmentId=p.value;refreshEnvUI()})
  }
  const w = document.querySelector('#workflowEnvironment')
  if (w && !w.dataset.bound) {
    w.dataset.bound='1'
    w.addEventListener('change',()=>{const workflow=currentWorkflow();if(workflow)workflow.environmentId=w.value||null;refreshEnvUI()})
  }
  document.querySelectorAll('[data-env-sync]').forEach(button=>{
    if(button.dataset.bound)return
    button.dataset.bound='1'
    button.addEventListener('click',()=>{
      const env=project.environments.find(item=>item.id===button.dataset.envSync)
      if(!env||!canEdit())return
      env.status='syncing';refreshEnvUI()
      setTimeout(()=>{env.status='ready';refreshEnvUI()},650)
    })
  })
  ...
  if(add&&!add.dataset.bound){
    ...
    add.addEventListener('click',()=>{
      if(!canEdit())return
      const n=sharedEnvs(project).length+1
      project.environments.push({id:uid('env'),name:`venv-${n}`,python:'3.12',path:`.venv-${n}`,status:'ready',manager:'uv',scope:'project'})
      refreshEnvUI()
    })
  }
  const addWorkflow=document.querySelector('[data-env-action="add-workflow"]')
  if(addWorkflow&&!addWorkflow.dataset.bound){
    addWorkflow.dataset.bound='1'
    addWorkflow.addEventListener('click',()=>{
      if(!canEdit())return
      const workflow=currentWorkflow();if(!workflow)return
      const base=effectiveEnv(workflow,project)
      const n=workflowEnvs(project,workflow).length+1
      const suffix=n===1?'':`-${n}`
      const env={id:uid('env'),name:`${workflow.name}-venv${suffix}`,python:base?.python||'3.12',path:`.venv-${workflow.id}${suffix}`,status:'ready',manager:'uv',scope:'workflow',workflowId:workflow.id}
...
const css=`.env-card{grid-column:1/-1}.env-card .card-head small{display:block;margin-top:3px}.env-list{display:grid;gap:8px;margin-top:14px}.env-row{display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:12px;align-items:center;padding:11px 12px;border:1px solid var(--border,#26324a);border-radius:10px}.env-row.default{outline:1px solid rgba(99,102,241,.55)}.env-row small{display:block;margin-top:3px}.env-scope{opacity:.65}.env-status{font-size:12px;font-weight:700}.env-status.ready{color:#49c77d}.env-status.syncing{color:#f0b84b}.env-status.broken{color:#f06b6b}.workflow-env{margin:12px 0 16px;padding:12px;border:1px solid var(--border,#26324a);border-radius:10px}.workflow-env-head{display:flex;justify-content:space-between;align-items:center;margin-bottom:10px}.workflow-env-head small{color:#49c77d}.workflow-env label{display:block}.workflow-env select{width:100%;margin-top:5px}.workflow-env-active{display:flex;gap:7px;align-items:center;margin:8px 0 10px}.workflow-env-active small{opacity:.72}.workflow-env>button{width:100%}.workflow-env .hint{display:block;margin-top:8px}.env-dot{width:8px;height:8px;border-radius:50%;display:inline-block;flex:0 0 auto}.env-dot.ready{background:#49c77d}.env-dot.syncing{background:#f0b84b}.env-dot.broken{background:#f06b6b}@media(max-width:900px){.env-row{grid-template-columns:1fr auto}.env-row button{grid-column:1/-1}}`
