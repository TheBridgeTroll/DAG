...
function bindEnv() {
  ...
  if (p && !p.dataset.bound) {
    p.dataset.bound = '1'
    p.addEventListener('change', () => { project.environmentId = p.value; inject() })
  }

  const w = document.querySelector('#workflowEnvironment')
  if (w && !w.dataset.bound) {
    w.dataset.bound = '1'
    w.addEventListener('change', () => { const workflow=currentWorkflow(); if(workflow) workflow.environmentId = w.value || null; inject() })
  }
  ...
  document.querySelectorAll('[data-env-sync]').forEach(button => {
    ...
    button.addEventListener('click', () => {
      const env = project.environments.find(item => item.id === button.dataset.envSync); if (!env || !canEdit()) return
      env.status = 'syncing'; inject()
      setTimeout(() => { env.status = 'ready'; document.querySelector('.env-card')?.remove(); inject() }, 650)
    })
  })
...
const observer = new MutationObserver(() => {
  if (queued) return
  queued = true
  queueMicrotask(() => { queued = false; inject() })
})
